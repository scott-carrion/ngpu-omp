...
void loadGridHelper (Grid_tc<T> & grid, std::ifstream & file, U * data, int n,
    int bytes, bool swap)
{
    if ( n < 0 ) n = 1;
...
Grid_tc<float> loadGrid (const std::string & fn)
{
    ...
    // create grid object
    // Doing the parameterized constructor's job here, since it's not allowed in order to be trivial
    Grid_tc<float> grid;

    /* BEGIN MANUAL PARAMETERIZED CONSTRUCTOR FOR GRID */
    grid.nr = info.nlines;
    ...
    if (grid.coordinateSys == nullptr) { grid.coordinateSys = new char[10000]; } // Using arbitrary large size for simplicity of implementation. Not a good idea, but -O3 should fix this for me

    // set default band names
    grid.bandNames = new char*[grid.nb];
    for (int j = 0; j < grid.nb; j++) { grid.bandNames[j] = new char[10000]; }  // Same here!
    ...
    for ( i = 0; i < grid.nb; i++ ) {
        ...
        ss << "Band " << i + 1;
        strcpy(grid.bandNames[i], ss.str().c_str());
    }

    /* END MANUAL PARAMETERIZED CONSTRUCTOR FOR GRID */
...
// swap   : true if a byte swap is necessary.
template <class T, class U>
void loadBandHelper (Grid_tc<T> & grid, int k, int ngrids, std::ifstream & file,
    U * data, int bytes, bool swap)
{
    ...
    if ( grid.get_interleave() == "bsq" ){
        ...
        loadGridHelper(grid, file, data, 100, bytes, swap);  // FIXME XXX (need to remove?) using very small size here
    }
...
//      of bands.
// -- Returns --
// A floating point Grid_tc object.
Grid_tc<float> loadBand (const std::string & fn, int k)
{
    // read header information
    HeaderInfo info = read_ENVIheader(fn);
    ...
    if (grid.coordinateSys == nullptr) { grid.coordinateSys = new char[10000]; } // Using very large size for simplicity in implementation. -O3 flag should fix this!

    // set default band names
    //bandNames = new std::string[nb];
    grid.bandNames = new char*[grid.nb];
    for (int j = 0; j < grid.nb; j++) { grid.bandNames[j] = new char[10000]; }
    ...
    grid.set_noData(info.nodata);
    const char* tmp_ptr = info.interleave.c_str();  // this might break
...
template <class T> int ENVI_dataType_code ( const Grid_tc<T> & grid )
{
    return 4; // 32-bit float -- DEFAULT
}
template <> int ENVI_dataType_code ( const Grid_tc<unsigned char> & grid )
{
    return 1; // 8-bit byte (unsigned int)
}
template <> int ENVI_dataType_code ( const Grid_tc<short> & grid )
{
    return 2; // 16-bit int
}
template <> int ENVI_dataType_code ( const Grid_tc<int> & grid )
{
    return 3; // 32-bit int
}
template <> int ENVI_dataType_code ( const Grid_tc<double> & grid )
{
    return 5; // 64-bit float
}
template <> int ENVI_dataType_code ( const Grid_tc<unsigned short> & grid )
{
    return 12; // 16-bit unsigned int
}
template <> int ENVI_dataType_code ( const Grid_tc<unsigned int> & grid )
{
    return 13; // 32-bit unsigned int
}
template <> int ENVI_dataType_code ( const Grid_tc<long> & grid )
{
    return 14; // 64-bit int
}
template <> int ENVI_dataType_code ( const Grid_tc<unsigned long> & grid )
{
...
template <class T>
void saveGrid ( Grid_tc<T> & grid, std::string filename )
{
    // open header file
...
template <class T, class U>
void convert ( const Grid_tc<T> & A, Grid_tc<U> * B )
{
    *B = Grid_tc<U>(A.ncols(), A.nrows(), A.nbands());
