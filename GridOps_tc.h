...
template <typename T>
void getNearest4_tc(const Grid_tc<T> & g, double i0, double j0, GridCell<T>* v, int nrows, int ncols, bool debug_flag) {
...
Grid_tc<T> skyview ( const Grid_tc<T> & dem, int daz, double r )
{
    ...
    int nrows = dem.nrows(); int ncols = dem.ncols();
...
#pragma omp target teams distribute parallel for private(flag, i, ii, j, jj, k, kk, a, d, sinAz, cosAz, Amax, sum, count, angle, z) shared(dem, interval, dmax, deg2rad, out, nrows, ncols) map(tofrom: out.data[0:out.get_volume()], dem.data[0:dem.get_volume()]) collapse(2)
    ...
    for ( i = 0; i < nrows; ++i ) {
		...
		for ( d = 1; d <= dmax; ++d ) { 
                    ...
                    flag = false;
                    if (d < 2) { getNearest4_tc(dem, ii, jj, cells, nrows, ncols, true); }
		    else { getNearest4_tc(dem, ii, jj, cells, nrows, ncols, false); }
		...
		for ( kk = 0; !flag && kk < 4; ++kk ){
                    ...
                    if (d < 2) { z = bilinearInterpolate_tc(ii, jj, cells, false); }  // FIXME: GPU VERSION SETS z TO 0 EVERY TIME
		    else { z = bilinearInterpolate_tc(ii, jj, cells, false); }

		    //if (d < 2) { printf("z value is %f\n", z); }

                    // track maximum angle from horizontal
                    angle = atan((z - dem[k]) / (d * dem_dx));
		    //if (d < 2) { printf("In kk loop, angle is: %f\n", angle); }
                    if ( angle > Amax ) { Amax = angle; /*printf("New Amax is %f\n", Amax);*/ }
                }  

                Amax = cos(Amax);
                sum += Amax * Amax;
            }
	    //printf("Assigning out[%d] == %f * %f == %f\n", k, sum, interval, sum * interval);
            out[k] = sum * interval;
        }
    }
    // std::cout << "\rskyview " << (static_cast<double>(count++) / dem.size()) << "%\n";
